# Glossary Documentation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Compile a comprehensive, alphabetically ordered glossary of OAG domain terms and wire it into the VitePress documentation navigation.

**Architecture:** Add one new content page `www/docs/docs/Glossary.md` that opens with a "Most relevant terms" section (the content merged in from the existing `Terms-you-need-to-understand.md`), followed by the full alphabetical glossary. The old `Terms-you-need-to-understand.md` page is removed and its sidebar entry is replaced by a "Glossary" entry under "Getting Started". Verification is the VitePress build itself — `ignoreDeadLinks` is set to `'localhostLinks'`, so any unresolved internal link (including a sidebar entry pointing to a missing page) fails the build.

**Tech Stack:** VitePress (Vue-based static site generator), Markdown, Node/npm. Docs live under `www/`; build with `npm run docs:build` from `www/`.

## Global Constraints

- Documentation must be in English. (CLAUDE.md Working Directives)
- Documentation represents the current state of the system — do NOT write change-history phrasing ("this was changed in task X", "before it was Y", "closes issue Z"). (CLAUDE.md Working Directives)
- Every internal doc link must resolve — the VitePress build fails on dead internal links because `ignoreDeadLinks: 'localhostLinks'` only exempts `localhost` URLs. (`www/docs/.vitepress/config.js:9`)
- Definitions must be grounded in how OAG actually uses each term (not generic textbook definitions).
- `node_modules` is already installed under `www/`; do not reinstall unless the build reports missing dependencies.

---

## File Structure

- **Create:** `www/docs/docs/Glossary.md` — a "Most relevant terms" section followed by the full alphabetically ordered glossary. Single responsibility: define OAG domain terms. Rendered by VitePress.
- **Modify:** `www/docs/.vitepress/config.js` — replace the existing "Terms you need to understand" sidebar item (under "Getting Started") with a "Glossary" item pointing to the new page.
- **Delete:** `www/docs/docs/Terms-you-need-to-understand.md` — its content is merged into the "Most relevant terms" section of the Glossary. Only the sidebar references it, so no doc-body links break.

---

### Task 1: Create the Glossary content page

**Files:**
- Create: `www/docs/docs/Glossary.md`
- Verify with: `cd www && npm run docs:build`

**Interfaces:**
- Consumes: nothing (self-contained content page).
- Produces: a doc page reachable at the route `/docs/Glossary` (VitePress derives the route from the file path). Task 2 links to this exact route `/Glossary` (relative to the `base: '/docs'` used in the sidebar group).

- [x] **Step 1: Write the failing test (build without the page, prove the route is absent)**

Run the build first to establish a green baseline, then confirm the target page does not yet exist. The "test" for a docs page is: the built output contains a `Glossary.html` file with the glossary's terms.

Run:
```bash
cd www && npm run docs:build && ls docs/.vitepress/dist/docs/Glossary.html
```
Expected: build succeeds, but the final `ls` FAILS with `No such file or directory` (the page does not exist yet).

- [x] **Step 2: Create the Glossary page**

Create `www/docs/docs/Glossary.md` with exactly this content:
...
- [x] **Step 3: Build the docs and verify the page is generated**

Run:
```bash
cd www && npm run docs:build && ls docs/.vitepress/dist/docs/Glossary.html
```
Expected: build succeeds with no dead-link errors, and the `ls` PRINTS `docs/.vitepress/dist/docs/Glossary.html` (page now exists).

- [x] **Step 4: Verify both sections are present**

Confirm the page carries both the "Most relevant terms" section and the full "All terms" section:
```bash
cd www && grep -c "^## Most relevant terms$" docs/docs/Glossary.md && grep -c "^## All terms$" docs/docs/Glossary.md
```
...
```bash
cd /home/kali/IdeaProjects/Application-Gateway
git add www/docs/docs/Glossary.md
git commit -m "docs: add OAG glossary page"
```

---

### Task 2: Swap the Terms page for the Glossary in navigation

**Files:**
- Modify: `www/docs/.vitepress/config.js:43-46` (the "Getting Started" sidebar `items` array)
- Delete: `www/docs/docs/Terms-you-need-to-understand.md`
- Verify with: `cd www && npm run docs:build`

**Interfaces:**
- Consumes: the route `/Glossary` produced by Task 1 (relative to the sidebar group's `base: '/docs'`).
- Produces: a sidebar entry "Glossary" under "Getting Started", replacing the former "Terms you need to understand" entry. No page links to `/docs/Terms-you-need-to-understand` after this task.

- [x] **Step 1: Write the failing test (assert navigation still points at the old page)**

Run:
```bash
cd www && grep -c "Terms-you-need-to-understand" docs/.vitepress/config.js
```
Expected: prints `1` (the sidebar still references the page being removed). Treat this non-zero result as the failing (red) state — navigation must no longer reference the old page.

- [x] **Step 2: Replace the sidebar entry**

In `www/docs/.vitepress/config.js`, in the "Getting Started" group, change the `items` array from:
...
- [x] **Step 3: Delete the merged Terms page**

Run:
```bash
cd /home/kali/IdeaProjects/Application-Gateway
git rm www/docs/docs/Terms-you-need-to-understand.md
```
Expected: `rm 'www/docs/docs/Terms-you-need-to-understand.md'`.

- [x] **Step 4: Verify navigation is updated and the site builds with no dead links**

Run:
```bash
cd www && grep -c "Terms-you-need-to-understand" docs/.vitepress/config.js; grep -c "Glossary" docs/.vitepress/config.js && npm run docs:build 2>&1 | grep -i "dead link" || echo "BUILD OK, NO DEAD LINKS"
```
Expected: the first `grep -c` prints `0` (old page no longer referenced), the second prints `1` (Glossary entry present), and `BUILD OK, NO DEAD LINKS` is printed.

- [x] **Step 5: Commit**

```bash
cd /home/kali/IdeaProjects/Application-Gateway
git add www/docs/.vitepress/config.js
git commit -m "docs: replace terms page with glossary in navigation"
```
...
**1. Spec coverage:** The spec ("compile a glossary and add it to the VitePress documentation; merge the Terms page into the Glossary as a first 'Most relevant terms' section") is covered: Task 1 creates the Glossary with the "Most relevant terms" section first (carrying the five terms from the old Terms page verbatim) followed by the full alphabetical glossary; Task 2 wires it into navigation and removes the merged Terms page.

**2. Placeholder scan:** The full glossary content (both sections) is inlined in Task 1 Step 2; the exact config edit is inlined in Task 2 Step 2. No `TBD`/`TODO`/"add appropriate…" placeholders.

**3. Type/name consistency:** The route produced by Task 1 (`/Glossary` under `base: '/docs'`) matches the sidebar `link: '/Glossary'` in Task 2 Step 2. No content links to `/docs/Terms-you-need-to-understand` after the merge (only the sidebar did, and Task 2 removes it), so deleting the page in Task 2 Step 3 leaves no dead links. Verification commands consistently reference `docs/docs/Glossary.md`, `docs/.vitepress/dist/docs/Glossary.html`, and `docs/.vitepress/config.js` relative to `www/`.

## Notes on grounding

Definitions are grounded in the existing docs and source:
- Routes / `allowAnonymous` / `path` / `rewrite`: `www/docs/docs/Configuration-Routes.md`
- Security profiles / predefined profiles / CSRF strategies / response headers: `www/docs/docs/Configuration-SecurityProfiles.md`, `Csrf-Protection.md`
- Login providers / OIDC / GitHub / scopes / federated logout: `www/docs/docs/Configuration-Login-Providers.md`
- User mapping / JWT claims / mapping syntax: `www/docs/docs/Configuration-User-Mapping.md`
- Key management / JWT signer / rotation / stores: `www/docs/docs/Key-management-and-JWT-signer.md`
- JWKS endpoint / JWK / kid: `www/docs/docs/JWKS-JWT-signing-public-keys-of-OAG.md`
- Tracing / trace profile / W3C Trace Context / correlation logging: `www/docs/docs/Tracing-Log-Correlation-Correlation-Logging.md`, `Terms-you-need-to-understand.md`
- Session behaviour / rolling session: `www/docs/docs/Configuration-Session-Behaviour.md`
- Session blacklist / persistent map / login state: `oag/src/main/java/org/owasp/oag/services/blacklist/`, `oag/src/main/java/org/owasp/oag/persistentmap/`, `oag/src/main/java/org/owasp/oag/services/login/drivers/LoginDriverResult.java`
